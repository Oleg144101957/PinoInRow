...
import com.pl.nk.col.lec.ui.theme.DefCorner
...
fun TextButton(
    ...
    Button(
        ...
        modifier = modifier.background(Gradient, DefCorner),
        enabled = enabled
    ) {
        Text(
            text,
...
fun IconButton(
    ...
    IconButton(
        onClick = { onClick() },
        modifier = modifier
            .background(Gradient, DefCorner)
            .padding(4.dp),
    ) {
        Icon(painterResource(iconRes), contentDescription = iconRes.toString(), tint = Color.White)
    }
}
